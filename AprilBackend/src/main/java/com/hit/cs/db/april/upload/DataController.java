		...
		if (type.equals("0")) {
			for (int i = 0; i < files.size(); i++) {
				if (!files.get(i).isEmpty()) {
					...
					try (FileOutputStream fileOuputStream = new FileOutputStream(Constants.APRILBACKEND_RESOURCES_FILE 
					...
					for (String db : database) {
						String[] args = new String[] { Constants.PYTHON_PATH, Constants.PYTHON_PROJECT_DBDQN_SRC + "init_lubm_data.py", 
								Constants.APRILBACKEND_RESOURCES_FILE + files.get(i).getOriginalFilename(),
								db };
