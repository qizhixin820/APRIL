...
if __name__ == '__main__':
    ...
    agent = DQN(state_dim, action_dim, MyLogger(path='../data/qindex_train_dqn_log'+date+'.txt'), model_number)
    ...
    for episode in range(episodes):
        ...
        while t == False and step < max_step_per_episode:
            ...
            s, r, t = env.step(a)  
            r = r * 100 # env中将reward换算为ms单位后, 再乘100
        ...
        # 每4轮保存模型核mydb2的成员变量
        if episode % 10 == 0:
            agent.save_models(episode if model_number is None else (episode+model_number))
            env.save_myindex(episode if model_number is None else (episode+model_number))
